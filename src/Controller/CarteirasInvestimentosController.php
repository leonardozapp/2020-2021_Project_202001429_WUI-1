...
class CarteirasInvestimentosController extends AppController
{
    ...
    public function indicadores($id = null)
    {
        ...
        foreach ($carteirasInvestimento->operacoes_financeiras as $operacao) {
            ...
            $busca_rentabilidade = $this->CarteirasInvestimentos->OperacoesFinanceiras->CnpjFundos->DocInfDiarioFundos->find(
                'all',
                ['order' => ['DT_COMPTC' => 'ASC']]
            )->where(['cnpj_fundo_id' => $fundo_id, 'DT_COMPTC >=' => $primeira_data_carteira]);
